...
(defun tlon-read-bare-dirs ()
  "Read bare directory translations from JSON file."
  (let* ((repo (tlon-repo-lookup :dir :name "babel-refs"))
	 (file (file-name-concat repo "languages" "folder-translations.json"))
	 (json (tlon-read-json file))
         bare-dirs)
    (dolist (cat '("articles" "tags" "authors" "collections"))
      (when-let ((translations (alist-get cat json nil nil #'string=)))
        (let ((dir-pairs (list (cons "en" cat))))
          (dolist (lang '("es" "fr" "it" "ar" "ko" "ja"))
            (when-let ((trans (alist-get lang translations nil nil #'string=)))
              (push (cons lang trans) dir-pairs)))
          (push (nreverse dir-pairs) bare-dirs))))
    (nreverse bare-dirs)))

(defconst tlon-core-bare-dirs (tlon-read-bare-dirs)
...
(defun tlon-read-language-properties ()
  "Read language properties from JSON file."
  (let* ((repo (tlon-repo-lookup :dir :name "babel-refs"))
         (file (file-name-concat repo "languages" "language-translations.json"))
         (json (tlon-read-json file))
         properties)
    (dolist (entry (reverse json))
      (let* ((lang (car entry))
             (props (cdr entry))
             (plist (list :name lang))
             (standard (alist-get "standard" props nil nil #'string=))
             (code (alist-get "code" props nil nil #'string=))
             (locale (alist-get "locale" props nil nil #'string=))
             (iso-639-2 (alist-get "iso-639-2" props nil nil #'string=)))
        (when standard (setq plist (plist-put plist :standard standard)))
        (when code (setq plist (plist-put plist :code code)))
        (when locale (setq plist (plist-put plist :locale locale)))
        (when iso-639-2 (setq plist (plist-put plist :iso-639-2 iso-639-2)))
        (push plist properties)))
    properties))

(defconst tlon-languages-properties (tlon-read-language-properties)
