...
+ *From the Command Line (Batch Mode)*: This is suitable for automated checks (e.g., in scripts or CI pipelines). You need to explicitly add the necessary directories to the `load-path` using =--eval= *before* loading files. Load the main package file (=tlon.el=) first to set up dependencies before loading the test file. Use full paths for loaded files.
  #+begin_src shell
  emacs --batch \
    --eval '(add-to-list 'load-path "/path/to/tlon/directory/")' \
    --eval '(add-to-list 'load-path "/path/to/parent/repos/directory/")' \
    --load /path/to/tlon/directory/tlon.el \
    --load /path/to/tlon/directory/tlon-tts-tests.el \
