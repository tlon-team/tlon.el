...
+ *From the Command Line (Batch Mode)*: This is suitable for automated checks (e.g., in scripts or CI pipelines). You need to add the directory containing the source and test files to the load path using =-L=.
  #+begin_src shell
  emacs --batch -L /path/to/tlon/directory/ --load tlon-tts-tests.el --funcall ert-run-tests-batch-and-exit
  #+end_src
  Replace =/path/to/tlon/directory/= with the actual path to the directory containing =tlon-tts.el= and =tlon-tts-tests.el=. Emacs will exit with status 0 if all tests pass, and non-zero otherwise.
