...
+ *From the Command Line (Batch Mode)*: This is suitable for automated checks (e.g., in scripts or CI pipelines). You need to explicitly add the necessary directories to the `load-path` using a single =--eval= argument containing a =progn= block *before* loading files. Load the main package file (=tlon.el=) first to set up dependencies before loading the test file. Use full paths for loaded files. Run the command as a single line to avoid shell interpretation issues.
  #+begin_src shell
  emacs --batch --eval '(progn (add-to-list (quote load-path) "/path/to/tlon/directory/") (add-to-list (quote load-path) "/path/to/parent/repos/directory/"))' --load /path/to/tlon/directory/tlon.el --load /path/to/tlon/directory/tlon-tts-tests.el --funcall ert-run-tests-batch-and-exit
  #+end_src
