...
+ ~tlon-get-file-as-string~: Reads the content of a file into a string. Handles PDF conversion (to Markdown) and HTML rendering (to text using =shr= without displaying the intermediate buffer) internally.
