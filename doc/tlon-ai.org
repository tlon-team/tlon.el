...
:END:
#+findex: tlon-ai-get-help
The command ~tlon-ai-get-help~ asks a question about the tlon repository and related tools, using documentation files as context. It prompts the user for a question, finds relevant =.org= files in the main =tlon.el= documentation directory (=doc/=) and the =dotfiles/emacs/extras/doc/= directory, adds these files to the =gptel= context, and sends the request to the AI. The answer is displayed in a new buffer.
