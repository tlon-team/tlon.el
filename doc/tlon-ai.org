...
When invoked, it prompts the user for a file to process. It then sends a request to the AI, instructing it to act as an academic editor. The AI is granted access to a specific set of tools to complete its task:
