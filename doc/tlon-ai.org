...
+ ~tlon-ai-summarization-model~ :: Model for summarizing text (e.g., generating abstracts, synopses). It's recommended to use a model with a large context window.
  #+begin_src emacs-lisp
  (setq tlon-ai-summarization-model '("Gemini" . gemini-flash-latest))
  #+end_src
...
+ ~tlon-ai-markdown-fix-model~ :: Model for fixing Markdown formatting issues, especially when comparing original and translated documents.
  #+begin_src emacs-lisp
  (setq tlon-ai-markdown-fix-model '("Gemini" . gemini-flash-latest))
  #+end_src
...
+ ~tlon-ai-summarize-commit-diffs-model~ :: Model for summarizing commit diffs.
  #+begin_src emacs-lisp
  (setq tlon-ai-summarize-commit-diffs-model '("Gemini" . gemini-pro-latest))
  #+end_src
...
+ ~tlon-ai-help-model~ :: Model to use for the AI help command (~tlon-ai-ask-for-help~). The value is a cons cell whose car is the backend and whose cdr is the model itself. See ~gptel-extras-ai-models~ for the available options. If nil, use the default ~gptel-model~.
  #+begin_src emacs-lisp
  (setq tlon-ai-help-model '("Gemini" . gemini-flash-lite-latest))
  #+end_src
...
The command ~tlon-ai-propagate-changes~ attempts to automatically apply changes made in a commit to corresponding files in other ~tlon~ content repositories (originals and translations). If invoked from =magit-status-mode= with point on a commit, that commit is used; otherwise, the latest commit in the current repository is used.

1. It identifies the current repository and the commit to use: if invoked from =magit-status-mode= with point on a commit, it uses that commit; otherwise, it uses the latest commit.
