...
8. It saves the images sequentially in the target directory (e.g., =figure-01.png=, =figure-02.png=, etc.). The file extension is determined by first checking the image's file signature ("magic numbers"), including detecting SVG by the presence of an XML prolog or an <svg> tag. If that fails, it inspects the =Content-Type= HTTP header and normalizes subtypes like =image/svg+xml= to =svg= and =image/jpeg= to =jpg=; as a final fallback, it uses the file extension from the URL itself (also normalized). If no extension can be determined, the image is skipped and processing continues.
