...
10. It replaces each Markdown image with a <Figure> tag whose src points to the downloaded local file and whose alt is populated from the Markdown alt text when present. Extra blank lines around the inserted tag are avoided.
