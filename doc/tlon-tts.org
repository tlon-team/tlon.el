...
#+vindex: tlon-tts-narrate-only-missing-chunks
+ ~tlon-tts-narrate-only-missing-chunks~ :: Boolean. If non-nil (the default), `tlon-tts-narrate-staged-buffer` will only generate audio for chunks whose corresponding files do not already exist, effectively resuming an interrupted process. If nil, it will regenerate all chunks.
