...
+ ~tlon-tts-calculate-chunks~ :: Calculates the engine's character limit and calls ~tlon-tts-read-into-chunks~. This function ensures that chunking logic operates on text free of pre-existing `<!-- Chunk N -->` comments by temporarily removing them before `tlon-tts-read-into-chunks` is called.
