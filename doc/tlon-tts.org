...
+ ~tlon-tts-break-into-chunks~ :: Core logic for splitting text into chunks based on paragraph boundaries, engine character limits, and voice changes specified by =tlon-tts-voice-chunks=. It reads the paragraph number =N= from the =<!-- Paragraph N -->= comment preceding the start of each chunk's text and stores this number in the chunk data. This ensures the paragraph number used for filenames and internal processing matches the visible comment.
...
+ ~tlon-tts--generate-single-paragraph-by-number~ :: Internal function called by ~tlon-tts-narrate-staged-paragraphs~. Takes a 1-based paragraph number (read from the comment), finds the corresponding chunk data in ~tlon-tts-chunks~ (matching based on the paragraph number stored within the chunk), and calls ~tlon-tts-execute-generation-request~ to generate the audio for that specific chunk's text.
...
+ ~tlon-tts-get-chunk-name~ :: Generates the filename for an audio chunk based on the original filename and the paragraph number stored in the chunk data (e.g., =basename-paragraph-XXX.ext=). This number originates from the =<!-- Paragraph N -->= comment.
