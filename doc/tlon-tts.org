...
#+vindex: tlon-tts-commit-and-push-moved-audio
+ ~tlon-tts-commit-and-push-moved-audio~ :: Boolean. If non-nil (the default), `tlon-tts-move-file-to-audio-server` will also stage, commit, and push the moved audio file to the remote repository. If nil, only the file move is performed.
