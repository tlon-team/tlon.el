...
+ ~tlon-tts-narrate-staged-buffer~ :: (Autoloaded) Narrates the content currently prepared in the staging buffer.
  ...
  6. Once all chunks are generated, no automatic finalization is performed. Use ~tlon-tts-finalize-audio-processing~ to normalize (if ~tlon-tts-normalize-audio~ is non-nil), then append silence (if needed), and join the chunks into the final output file.
...
+ ~tlon-tts-finalize-audio-processing~ :: (Autoloaded) Manually triggers the final audio processing steps for a given base audio file. This includes normalizing chunk audio files first (if ~tlon-tts-normalize-audio~ is non-nil), then appending silence (if applicable for the engine), and joining them into the final output file (e.g., =filename.mp3=). It also handles deleting original chunk files if ~tlon-tts-delete-file-chunks~ is set, and opens Dired and the final audio file. This command is useful after selectively regenerating chunks with ~tlon-tts-narrate-staged-chunks~.
...
+ ~tlon-tts-finish-processing~ :: Finalizes the TTS process after all API chunks are generated. It normalizes chunks asynchronously (if enabled), then (for engines that need it) appends silence and joins the chunks. Handles cleanup of temporary files and opens the final audio file.
