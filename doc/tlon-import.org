...
+ ~tlon-pandoc-to-markdown~ :: The core Pandoc command arguments for converting HTML to Tlön-flavored Markdown (raw HTML, native divs/spans, fenced divs/spans, header attributes, fenced code blocks, no wrapping, strip comments).
