...
+ ~tlon-db-sync-confirm-threshold~ :: Minimum number of created/modified entries
  that triggers a confirmation prompt and brings the ~*Db Sync Log*~ buffer to
  the foreground after syncing.  Default is 10.
...
+ ~tlon-db-post-entry~ :: If called interactively, posts the BibTeX entry at point to the =/api/entries= endpoint. If called from Lisp with a =KEY=, it posts the corresponding entry. Requires authentication. The content is sent as =text/plain=. On success, it replaces the local BibTeX entry with the version returned by the API, which may have reordered fields. If the API responds with a 400 error because one or more author names are missing from the database, the command offers to create the missing names automatically via ~tlon-db-set-name~ and, if confirmed, retries the post once. Interactive prompts and error messages include the entry key and the author field for context. During automatic synchronization, the command does not prompt; entries with missing author names are skipped and a detailed line, including the entry key and the author field, is written to the ~*Db Sync Log*~ buffer. If the operation is not successful (HTTP status is not 200) or if ~tlon-debug~ is non-nil, displays the server's response in a temporary buffer named ~*Db API Result*~. Otherwise, a success message is shown. Returns ~t~ on success, ~nil~ otherwise.
...
+ ~tlon-db-move-entry-pair~ :: Moves the original entry and all its translations to the database, without prompting for a language. It determines the original from context (if point is on a translation, it finds its original). Entries already in the database are skipped.
...
3. For each change, the corresponding API endpoint is called:
   - *Additions and Modifications*: The entry is posted to the API using ~tlon-db-post-entry~. On success, the function updates the entry in both ~tlon-db-file-db~ and ~tlon-db-file-db-upstream~ with the version returned by the API. If the server rejects an entry due to missing author names, the synchronization skips that entry without prompting and logs a 'Skipped (missing authors)' line containing the entry key and the author field.
