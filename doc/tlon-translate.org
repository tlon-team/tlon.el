...
+ ~tlon-translate-remove-duplicate-abstract-translations~ :: Remove abstract translations from the per-language JSON files under =babel-refs/json/<code>/abstract-translations.json= when a corresponding translation entry exists in =db.bib= (detected via the =translation= and =langid= fields). Reports how many items were pruned and emits progress logs during the scan.
