...
+ ~tlon-translate-abstract-here~ :: Translate the abstract of the entry at point. If the entry is a DB translation missing an abstract, the original’s abstract is translated into the entry’s language and written into =db.bib=. Otherwise, you select a target language; if a DB translation entry exists for that language, the translated abstract is written into that entry’s =abstract= field in =db.bib= (prompting before overwriting). If no such entry exists, it is stored in =abstract-translations.json=. When both the source and target are in =tlon-project-languages=, a glossary is required; otherwise the translation proceeds without one.
