...
The command ~tlon-check-urls-in-file~ (Autoloaded) checks all URLs found in a specified =FILE= (or the current buffer's file) for dead links using the external =lychee= tool asynchronously.
1. Ensures the =lychee= executable is available.
2. Identifies the input file.
3. Extracts all URLs from the file using ~tlon-get-urls-in-file~.
4. Writes the URLs to a temporary file, one URL per line.
5. Constructs the =lychee= command to check the URLs from the temporary file.
6. Starts the =lychee= process asynchronously using =start-process-shell-command=, with output directed to a new buffer named =*lychee-output*=.
7. Sets a process sentinel:
   - When the process finishes, it displays a completion message.
   - It displays the output buffer.
   - It enables =goto-address-mode= in the results buffer, making URLs clickable.
   - It deletes the temporary file containing the list of URLs.
