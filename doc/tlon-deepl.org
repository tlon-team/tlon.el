...
+ ~tlon-deepl-translate-abstract~ :: Translates the =abstract= field of a BibTeX entry =KEY= into relevant languages.
  - Determines context (key, abstract text, source language code) using ~tlon-deepl--get-abstract-context~.
  - If called interactively, delegates to ~tlon-deepl--translate-abstract-interactive~ which prompts for a single target language and translates.
  - If called non-interactively, delegates to ~tlon-deepl--translate-abstract-non-interactive~ which iterates through all target languages, checks for existing translations in the JSON file, and translates missing ones.
...
+ ~tlon-deepl-print-translation~ :: Default callback for translation requests. Parses the JSON response (assuming the buffer content is UTF-8), extracts the translated text, and either messages it or copies it to the kill ring if =COPY= is non-nil.
...
+ ~tlon-deepl--get-abstract-context~ :: Internal helper for ~tlon-deepl-translate-abstract~. Determines the BibTeX key, abstract text, and source language code based on arguments and interactive context. Returns a list =(KEY TEXT SOURCE-LANG-CODE)= or nil.
