...
+ ~tlon-deepl-translate-abstract~ :: Translates the =abstract= field of a BibTeX entry =KEY= into all relevant ~tlon-project-target-languages~.
  - *Interactive Use*: If called interactively without =KEY= or =ABSTRACT= arguments, it determines the key and abstract from the current context (Ebib or BibTeX mode). It fetches the source language using the key.
  - *Interactive Use*: If called interactively without =KEY= or =ABSTRACT= arguments, it determines the key and abstract from the current context (Ebib or BibTeX mode). It fetches the source language using the key. It then prompts the user for a single target language and initiates translation for that language only.
  - *Non-Interactive Use*: If called programmatically (e.g., from ~tlon-deepl-translate-missing-abstracts~ or advice), the =KEY= argument *must* be provided. If the =ABSTRACT= argument is nil, it fetches the abstract text using the =KEY= (prioritizing the active Ebib buffer if the key matches, otherwise using ~tlon-tex-get-field-value-from-files~). It fetches the source language similarly. It proceeds directly without interactive prompts.
  - *Process*:
    1. Determines key, abstract text, and source language code based on context/arguments, using ~tlon-tex-get-field-value-from-files~ as a fallback/primary method for non-interactive use.
    2. If interactive, prompts for one target language and translates.
    3. If non-interactive, iterates through ~tlon-project-target-languages~.
    4. For each target language different from the source language:
       - *Checks* if a translation already exists for the =KEY= and =target-lang= in the JSON file (~tlon-file-abstract-translations~) using the internal helper ~tlon-deepl--get-existing-translation~.
       - If the translation is missing or empty in the JSON file:
         - Messages the intent to translate for the specific language.
         - Calls ~tlon-deepl-translate~ with the (brace-removed) abstract text, target language code, source language code.
         - Uses ~tlon-translate-abstract-callback~ as the callback to save the translation.
         - Passes =no-glossary-ok= to avoid glossary prompts in batch mode.
    5. Messages which specific languages translation was initiated for.
