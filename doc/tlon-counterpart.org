...
+ ~tlon-get-counterpart~ :: (Autoloaded) Calculates and returns the full path to the counterpart file of =FILE= (or the current buffer's file).
  - If the file is in a repository marked as =translations= (based on its =:subtype= property), it mirrors the file's relative directory into the originals repository and returns the unique Markdown file found there (prompting when multiple exist).
  - If the file is in a repository marked as =originals=, it prompts the user to select a target language, mirrors the relative directory into the translations repository, and selects the Markdown file found there (prompting when multiple exist).
...
+ ~tlon-get-counterpart-in-translations~ :: Logic for a translation file.  It determines the counterpart directory and returns the unique Markdown file located there, or prompts when several exist.

#+findex: tlon-get-counterpart-in-originals
+ ~tlon-get-counterpart-in-originals~ :: Logic for an original file.  It mirrors the relative path into the selected translations repository and returns the Markdown file present there, prompting when multiple exist.
