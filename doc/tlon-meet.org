...
+ ~tlon-meet-transcribe-audio~ :: Transcribes an =AUDIO-FILE= using =whisperx= with diarization and then formats the transcript using AI.
  ...
  6. If the transcript exists, it deletes other generated files (=.vtt=, =.srt=, =.tsv=, =.json=) and then calls ~tlon-meet-format-transcript~ to generate a formatted Markdown file (=.md=) using the AI.
