...
+ ~tlon-meet-format-transcript-command~ :: Interactively prompts for a transcript file (=.txt=) and participants, then calls the AI to format it, saving the result as a Markdown file (=.md=), deleting the original =.txt= file, and finally running the AI cleanup step on the =.md= file. This is useful if you already have a raw transcript and just need to format and clean it.

#+findex: tlon-meet-cleanup-transcript-command
+ ~tlon-meet-cleanup-transcript-command~ :: Interactively prompts for a formatted transcript file (=.md=) and participants, then calls the AI to clean up potential transcription errors, overwriting the file with the result.
...
+ *Meetings*:
  ...
  + =g= :: Group meeting (~tlon-create-or-visit-group-meeting-issue~)]
   ["Processing"
    ...
    ("c"   "cleanup transcript"         tlon-meet-cleanup-transcript-command)
...
+ ~tlon-meet-format-transcript~ :: Internal function called by ~tlon-meet-transcribe-audio~ and ~tlon-meet-format-transcript-command~. Takes a raw transcript file (=.txt=) and participants, calls the AI formatter, saves the result to a =.md= file, deletes the original =.txt= file, then calls ~tlon-meet-cleanup-transcript~, and finally calls the optional callback function after cleanup.
