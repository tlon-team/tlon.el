...
+ ~tlon-reconcile-all-issues-and-todos~ :: (Autoloaded) Reconciles all Org TODOs found in the relevant TODO files with their corresponding GitHub issues.
...
#+findex: tlon-reconcile-estimate
+ ~tlon-reconcile-estimate~ :: (Autoloaded) Synchronizes the estimate between the current issue (GitHub Project "Estimate" field) and its associated Org TODO ("Effort" property).
  - If only one has an estimate, the other is updated silently.
  ...
  - Works from either the issue buffer or the Org TODO heading.
