...
+ ~tlon-sync-issue-and-todo-from-issue~ :: Core sync logic called by ~tlon-sync-issue-and-todo~. Compares expected and actual TODO names and calls ~tlon-sync-issue-and-todo-prompt~ if they differ.
