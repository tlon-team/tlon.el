...
+ ~tlon-forg-sync-after-capture-project~ :: Controls whether ~tlon-sync-all-issues-in-project~ is run after ~tlon-capture-all-issues-in-project~ completes. Options:
  + =prompt= (default): Ask the user if they want to sync.
  + =t=: Always sync after capture.
  + =nil=: Never sync after capture.
  The sync operation will use the cached project item list if available, avoiding redundant API calls. Can be set temporarily via ~tlon-forg-menu~.
...
+ ~tlon-capture-issue~ :: (Autoloaded) Creates a new Org TODO item based on the specified =ISSUE= or one selected interactively. Accepts optional =INVOKED-FROM-ORG-FILE= and =OVERRIDE-STATUS= arguments.
  1. If =ISSUE= is provided, uses it. Otherwise, attempts to get the issue from the current context (e.g., a Forge buffer).
  2. If no issue can be determined from context (e.g., when called from an Org buffer not associated with a repository), it prompts the user to select a repository (from Tlön's known repos or by listing from GitHub via `gh` CLI for ~forge-extras-project-owner~) and then an issue from that repository.
  ...
  5. If the issue's title starts with "Job: ":
     ...
     - Stores or refiles the job TODO under its master job heading using ~tlon-store-or-refile-job-todo~. The job TODO is initially captured (potentially to =INVOKED-FROM-ORG-FILE=, if an Org file), and then immediately refiled to the central jobs file under its master heading.
  6. If it's a generic (non-job) issue:
     - The TODO is captured to the repository-specific Org file (e.g., =REPO-NAME.org= in the directory specified by =paths-dir-tlon-todos=) if that file exists. Otherwise, it's captured to the main generic TODOs file (see ~tlon-get-todos-generic-file~).
  If =OVERRIDE-STATUS= is provided (e.g., by ~tlon-create-issue-from-todo~), that status string is used for the TODO heading, bypassing a call to ~tlon-get-status-in-issue~. Only captures if a TODO doesn't already exist for the issue.
...
+ ~tlon-capture-all-issues-in-repo~ :: (Autoloaded) Captures all open issues in a selected repository that are not assigned to another user (behavior depends on ~tlon-when-assignee-is-someone-else~, which defaults to `no-capture` effectively in bulk mode).
  1. If a repository cannot be inferred from the current context, the user is prompted to select one (from Tlön's known repos or by listing from GitHub via `gh` CLI for ~forge-extras-project-owner~).
  ...
  5. If any TODOs were captured, the target Org file (repository-specific or generic) is opened, and point is moved to the last captured TODO using =org-capture-goto-last-stored=.
...
+ ~tlon-capture-all-issues-in-project~ :: (Autoloaded) Captures all open issues in the GitHub Project configured in =forge-extras.el= (via ~forge-extras-project-owner~ and ~forge-extras-project-number~) that are not assigned to another user.
  ...
  4. After all repositories are processed for capture, it may optionally call ~tlon-sync-all-issues-in-project~ based on the value of ~tlon-forg-sync-after-capture-project~. If called, the sync operation will use the project items list fetched during the capture phase, avoiding redundant API calls.
  5. Restores the original window configuration and clears the =org-refile= cache upon completion.
...
+ ~tlon-create-issue-from-todo~ :: (Autoloaded) Creates a new GitHub issue based on the Org heading at point. Verbose logging can be enabled with a prefix argument.
  1. Ensures the heading doesn't already have an issue number.
  2. Determines the repository:
     a. First, by checking if the current buffer's filename (e.g., =REPO.org=) corresponds to a known Tlön repository.
     b. If not found, by checking for a repository tag in the Org heading.
     c. If still not found, prompts the user to select a repository.
...
+ *Options*: (These set the corresponding user options temporarily for the session)
  ...
  + =-C= :: Sync after project capture (~tlon-forg-sync-after-capture-project-infix~)
...
+ ~tlon-get-repo-from-heading~ :: Extracts the repository identifier (e.g., [repo-id]) from the heading. It then attempts to look up the full directory path by first treating the identifier as an abbreviation, and if that fails, as a name.
...
+ ~tlon-make-todo-name-from-issue~ :: Constructs the full Org heading string for an =ISSUE=, including status, repo abbreviation, optional job action, the issue link, and tags. Handles =NO-ACTION=, =NO-STATUS=, and =OVERRIDE-STATUS= flags. If =OVERRIDE-STATUS= is provided, it's used directly for the status part of the heading, otherwise ~tlon-get-status-in-issue~ is called.
...
+ ~tlon-store-todo~ :: Uses =org-capture= with a specific =TEMPLATE= ("tbG" or "tbJ") to store a new TODO for the =ISSUE=, unless one already exists. Handles master job TODO creation. Accepts an optional =OVERRIDE-STATUS= to pass to ~tlon-make-todo-name-from-issue~.
...
+ ~tlon-forg-check-for-duplicate-todos~ :: Checks for duplicate TODO entries in a given =FILE= (defaulting to the current buffer's file). A duplicate is defined as more than one Org heading having the same =orgit-topic:= ID. If duplicates are found, it signals an error listing them. This is useful for ensuring data integrity before running commands that might fail with duplicate entries.
