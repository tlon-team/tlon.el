...
#+vindex: tlon-forg-project-number
+ ~tlon-forg-project-number~ :: The GitHub Project number (integer) used to fetch issue status and other project-specific fields. Defaults to 9.
#+vindex: tlon-forg-project-owner
+ ~tlon-forg-project-owner~ :: The GitHub owner (organization or user) for the project. This is used in GraphQL queries to fetch issue and project field details (e.g., for the project specified by ~tlon-forg-project-number~). Also used by ~tlon-forg-discover-project-node-ids~. Defaults to "tlon-team".
#+vindex: tlon-forg-project-node-id
+ ~tlon-forg-project-node-id~ :: Constant holding the global Relay Node ID of the target GitHub Project (e.g., Project #9). This should be configured with the correct ID for your primary project.
#+vindex: tlon-forg-status-field-node-id
+ ~tlon-forg-status-field-node-id~ :: Constant holding the global Relay Node ID of the "Status" field within the target project. This should be configured with the correct ID.
#+vindex: tlon-forg-status-option-ids-alist
+ ~tlon-forg-status-option-ids-alist~ :: Constant holding an alist mapping GitHub Project status names (e.g., "Doing") to their global Option IDs for the "Status" field in the target project. This should be configured with the correct alist.
#+vindex: tlon-forg-gh-project-query
+ ~tlon-forg-gh-project-query~ :: A format string containing the raw GraphQL query to fetch issue details. It's intended to be formatted with owner name, repository name, and issue number, and then passed to `gh api graphql -f query=-` via standard input.
#+vindex: tlon-gh-add-item-to-project-mutation-query
+ ~tlon-gh-add-item-to-project-mutation-query~ :: GraphQL mutation string to add an issue to a project.
#+vindex: tlon-gh-update-project-item-field-mutation-query
+ ~tlon-gh-update-project-item-field-mutation-query~ :: GraphQL mutation string to update a single-select field (like "Status") of a project item.
#+vindex: tlon-gh-field-ids
+ ~tlon-gh-field-ids~ :: Alist mapping field names (e.g., =:effortid=) to their GitHub ProjectV2 field IDs (experimental, largely superseded by more specific Node ID constants).
...
+ ~tlon-get-status-in-issue~ :: Returns the GitHub Project status of an =ISSUE= from the project specified by ~tlon-forg-project-number~, mapped to an Org TODO keyword (e.g., "DOING", "NEXT"). It fetches the 'Status' field via `gh` CLI. If the project status cannot be determined or the issue is not in the specified project, it falls back to a status based on the issue's open/closed state (e.g., "TODO", "DONE").
...
+ ~tlon-update-issue-from-todo~ :: Updates the GitHub issue (title, labels, project status) to match the Org TODO heading at point. If the issue is not in the target project (defined by ~tlon-forg-project-number~ and ~tlon-forg-project-node-id~), prompts to add it before setting the status. Uses `gh` CLI for project interactions. *Requires user configuration of Node ID constants.*
...
+ ~tlon-gh-parse-issue-fields~ :: Parses the raw JSON list returned by ~tlon-gh-get-issue-fields~ into a structured property list. This plist includes the issue's title, assignees, labels, effort, GitHub project status name, the issue's global Node ID, the project item's Node ID (if in the target project), the "Status" field's Node ID, and the selected status option's Node ID. It filters for data related to the project specified by ~tlon-forg-project-number~.
#+findex: tlon-gh--call-api-graphql-mutation
+ ~tlon-gh--call-api-graphql-mutation~ :: Internal helper function to execute a GitHub GraphQL mutation via `gh api graphql`, passing specified variables. Returns the parsed JSON response or nil on error.
#+findex: tlon-gh-add-issue-to-project
+ ~tlon-gh-add-issue-to-project~ :: Adds an issue (specified by its Node ID) to a project (specified by its Node ID) using a GraphQL mutation. Returns the new project item's Node ID or nil on failure.
#+findex: tlon-gh-update-project-item-status-field
+ ~tlon-gh-update-project-item-status-field~ :: Updates the "Status" field of a project item using a GraphQL mutation. Requires Node IDs for the project, project item, "Status" field (from ~tlon-forg-status-field-node-id~), and the target status option (from ~tlon-forg-status-option-ids-alist~).
#+findex: tlon-forg-discover-project-node-ids
+ ~tlon-forg-discover-project-node-ids~ :: Interactive command to help discover GitHub Project Node IDs for verification or for configuring a different project. It prompts for the owner and project number, then uses `gh` CLI commands to find and display the project's Node ID, the "Status" field's Node ID, and the Node IDs for status options. This command is for discovery and does not automatically set the global constants.
