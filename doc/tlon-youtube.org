...
+ ~tlon-youtube-upload-video~ :: Uploads a video file to YouTube using the YouTube Data API v3 resumable upload method. This command uses the ~curl~ command-line tool to perform a two-step upload process: first initializing the upload to get an upload URL, then uploading the actual video file. Prompts the user to select a video file (filtered to show only ~.mp4~ files), enter a title and description, and choose a privacy setting ("private", "unlisted", or "public"). Before execution, it prints both `curl` commands to the *Messages* buffer, which is useful for debugging. The resumable upload method is more reliable for larger files and easier to debug since you can test the video file upload directly. Requires ~tlon-youtube-client-id~ and ~tlon-youtube-client-secret~ to be configured for OAuth 2.0 authentication. This command can also be accessed via the [[h:tlon-youtube-menu][tlon-youtube-menu]].

#+findex: tlon-youtube-prepare-upload-command
+ ~tlon-youtube-prepare-upload-command~ :: Prepares and displays the `curl` commands for a YouTube video upload without executing them. This command is useful for debugging, as it allows you to run the two-step resumable upload process from a terminal. It prints both commands to the *Messages* buffer: the initialization command (which returns an upload URL) and the video upload command (which uploads the raw video file to that URL). This makes debugging much easier since you can test the video file upload directly with a simple `curl` command. The metadata is stored in a separate JSON file for the initialization step. This command can also be accessed via the [[h:tlon-youtube-menu][tlon-youtube-menu]].
