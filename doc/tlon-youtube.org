...
To resolve authentication issues, the authorization process has been automated. This requires a one-time update to your OAuth Client ID configuration in the Google Cloud Console. This change makes the process more reliable and removes the need for manual copy-pasting of authorization codes.
...
3. Find your existing **Web application** OAuth client ID and click the pencil icon to edit it. (If you don't have one, create a new one of type "Web application").
4. Under the *Authorized redirect URIs* section, click *+ ADD URI*.
5. In the text box that appears, enter **exactly** this value: `http://localhost:8080/auth-callback`
6. You can remove the old `http://localhost:8080` URI if it exists, to keep things clean.
7. Click *SAVE*. You do not need to create new credentials; just update the existing ones.
8. Ensure your `tlon-youtube-client-id` and `tlon-youtube-client-secret` in Emacs correspond to this client ID.
...
Once your credentials are configured correctly, the authorization process is fully automated within Emacs.

**Automated Workflow:**
1. Run ~M-x tlon-youtube-authorize~.
2. Emacs will start a temporary local web server.
...
5. After you approve, Google will redirect your browser to the local server.
6. Emacs will automatically capture the authorization code, exchange it for tokens, and shut down the server.
