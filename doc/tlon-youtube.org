...
4. After authorization, you'll be redirected to a localhost page that may show an error (this is normal)
5. Copy the authorization code from the URL in your browser's address bar (after "code=")
6. Paste the authorization code into Emacs when prompted
7. The system will automatically obtain and store both access and refresh tokens
