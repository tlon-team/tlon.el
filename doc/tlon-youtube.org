...
+ ~tlon-youtube-authorize~ :: Starts the OAuth 2.0 authorization flow for the YouTube API using a manual copy-paste method. This opens a browser window for you to authorize the application. You must then copy the authorization code from your browser and paste it back into Emacs. This command requires a **Desktop app** OAuth 2.0 client ID. This command can also be accessed via the [[h:tlon-youtube-menu][tlon-youtube-menu]].
...
To upload videos and thumbnails to YouTube, you must first authorize the Tlön application to access your Google Account. This is done using OAuth 2.0. The process has been simplified to a manual copy-and-paste flow, which is the most reliable method.

**IMPORTANT**: For this to work, you *must* use an OAuth 2.0 Client ID of type **Desktop app**. If you have been using a "Web application" client ID, it will not work with this authorization flow and you will get a `redirect_uri_mismatch` error.

Please follow these steps carefully to create the correct credentials:
...
Once your **Desktop app** credentials are configured correctly, follow these steps to authorize:

1. Run ~M-x tlon-youtube-authorize~.
2. Your web browser will open to a Google authorization page.
3. Sign in to the Google Account you want to use for YouTube uploads.
4. Grant the requested permissions to the application.
5. After you approve, Google will show you an authorization code.
6. Copy this code.
7. Switch back to Emacs.
8. Paste the code into the minibuffer prompt that says "Paste authorization code from browser here: " and press Enter.

Emacs will then exchange this code for an access token and a refresh token. The refresh token is stored securely and will be used to automatically get new access tokens when they expire. You should only need to perform this authorization process once.

**When you get a 401 error:** The system will automatically attempt to refresh your access token. If that fails (for example, if you revoked access in your Google Account settings), you will need to re-authorize by running ~M-x tlon-youtube-authorize~ again.
