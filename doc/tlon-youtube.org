...
#+findex: tlon-youtube-set-access-token
+ ~tlon-youtube-set-access-token~ :: Prompts the user to enter a new YouTube API access token. This sets the token for the current Emacs session, which can be useful if the existing token has expired. This command can also be accessed via the [[h:tlon-youtube-menu][tlon-youtube-menu]].
...
:PROPERTIES:
:CUSTOM_ID: h:tlon-youtube-access-token
:END:

**Important:** YouTube access tokens expire after 1 hour, so you'll need to refresh them regularly.

**Current workflow (manual):**
1. Go to the [[https://developers.google.com/oauthplayground/][Google OAuth 2.0 Playground]]
2. In "Step 1", find "YouTube Data API v3" and select the scope ~https://www.googleapis.com/auth/youtube.upload~
3. Click "Authorize APIs" and sign in with your Google account
4. In "Step 2", click "Exchange authorization code for tokens"
5. Copy the "Access token" and either:
   - Use ~M-x tlon-youtube-set-access-token~ to set it for the current session, or
   - Set the ~YOUTUBE_ACCESS_TOKEN~ environment variable

**When you get a 401 error:** This usually means your access token has expired. Simply repeat the above steps to get a new token.

**Future improvement needed:** The current implementation uses a simplified authentication approach that requires manual token management. A full implementation should handle the complete OAuth 2.0 flow within Emacs, including automatic token refresh using refresh tokens.
