...
+ ~tlon-read-abstract-translations~ :: Reads and parses the per-language JSON data for a language CODE from =babel-refs/json/<code>/abstract-translations.json= using ~tlon-read-json~. Returns an alist of (KEY . TEXT).
#+findex: tlon-write-abstract-translations
+ ~tlon-write-abstract-translations~ :: Writes the provided =DATA= (alist of (KEY . TEXT)) to the per-language JSON file for a language CODE using ~tlon-write-data~.
