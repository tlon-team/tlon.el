...
+ ~tlon-bib-replace-citations-max-concurrent~ :: Maximum number of Org files to process simultaneously when replacing citations with the AI agent (used by ~tlon-bib-replace-citations-in-org-files-in-directory~). Defaults to 5.
  #+begin_src emacs-lisp
  (setq tlon-bib-replace-citations-max-concurrent 5)
  #+end_src
...
+ ~tlon-bib-populate-journaltitle-fields-in-language~ :: Sets the =journaltitle= field for all BibTeX entries in the language of the entry at point. It computes the value once as ~(tlon-uqbar-front-get-message LANG "HomePage" "title")~, where =LANG= is the two-letter code resolved from the entry’s =langid=, then applies it to each matching entry (prompting before overwriting existing values). The set of keys is obtained from the same articles directory as above.
...
+ ~tlon-read-abstract-translations~ :: Reads and parses the per-language JSON data for a language CODE or standard language NAME from =babel-refs/json/<code>/abstract-translations.json= using ~tlon-read-json~. NAME is resolved to CODE via ~(tlon-lookup tlon-languages-properties :code :standard NAME)~. Returns an alist of (KEY . TEXT).
#+findex: tlon-write-abstract-translations
+ ~tlon-write-abstract-translations~ :: Writes the provided =DATA= (alist of (KEY . TEXT)) to the per-language JSON file for a language CODE or standard language NAME using ~tlon-write-data~. NAME is resolved to CODE via ~(tlon-lookup tlon-languages-properties :code :standard NAME)~.
#+findex: tlon-add-abstract-translation
+ ~tlon-add-abstract-translation~ :: Adds or updates a =TRANSLATION= for a given =KEY= in the per-language JSON store for =TARGET-LANG= (or an in-memory alist =VAR= when provided). Honors =OVERWRITE= when the key already exists.
#+findex: tlon-get-abstract-translation
+ ~tlon-get-abstract-translation~ :: Retrieves the translated abstract for a specific =BIBKEY= and language =CODE= from the per-language JSON file (=babel-refs/json/<code>/abstract-translations.json=). Returns the translation string or nil if not found.
...
+ ~tlon-bib-replace-citations-in-org-files-in-directory~ :: Runs ~tlon-bib-replace-citations-in-file~ on every =.org= file in a specified directory (non-recursively). Files are processed in batches of size ~tlon-bib-replace-citations-max-concurrent~, and the next batch starts only after all files in the current batch have finished processing.
