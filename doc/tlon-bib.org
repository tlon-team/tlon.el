...
+ ~tlon-bib-create-translations-from-dir~ :: Iterates over all Markdown files
  ...
  jumps to the corresponding BibTeX entry, calls
  ~tlon-create-bibtex-translation~, and then prompts to proceed to the next
  file. Stops when the user declines to continue.
