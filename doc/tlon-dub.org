...
+ ~tlon-dub-get-transcript~ :: (Autoloaded) Retrieves the transcript file (likely VTT or SRT format) for a specific language within an ElevenLabs dubbing project. Prompts for the =dubbing_id= and the desired =language_code= (e.g., "en", "es"). It displays the retrieved transcript content in a new buffer.
