...
+ ~tlon-dub--share-project-with-self~ :: (Internal) Shares a given resource ID (dubbing project ID) with the *current API key* (using the top-level =workspace_api_key_id= field in the JSON payload) granting the "editor" role. Called automatically after project creation. Returns =t= on success, =nil= on failure.
