...
#+findex: tlon-dub-generate-transcript-with-whisperx
+ ~tlon-dub-generate-transcript-with-whisperx~ :: (Autoloaded) Generates a transcript for a given audio or video file using the `whisperx` command-line tool. Prompts the user to select an =audio-file=. The command then runs `whisperx` with `--compute_type float32` and specifies the output directory (`--output_dir`) to be the same as the input file's directory. Output files (e.g., `.txt`, `.srt`, `.vtt`) are created by `whisperx` in that directory.
