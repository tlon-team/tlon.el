...
+ ~tlon-ebib--display-result-buffer~ :: Writes a =TITLE= and the content generated by =FORMATTER-FN= with =DATA= to the file specified by ~tlon-ebib-file-temp~ (using =iso-8-unix= encoding), and then opens this file. The =BUFFER-NAME= argument is ignored.
