...
+ ~tlon-ebib-post-entries~ :: Posts the content of the file specified by ~tlon-ebib-file-temp~ to the =/api/entries= endpoint. Requires authentication. The content is sent as =text/plain=. Displays the server's response (success message, validation errors, or other errors) in a temporary buffer named ~*Ebib API Result*~.

#+findex: tlon-ebib-check-name
+ ~tlon-ebib-check-name~ :: Checks if a given =NAME= exists in the API's database. Requires authentication. Displays the results (API message or error details) in a temporary buffer named ~*Ebib API Result*~.

#+findex: tlon-ebib-check-or-insert-name
+ ~tlon-ebib-check-or-insert-name~ :: Checks if a given =NAME= exists. If it doesn't exist and there are no similar conflicting names found by the API, it inserts the name. Otherwise, it displays the status (success, conflict, validation error) and any relevant details (messages, similar names) in a temporary buffer named ~*Ebib API Result*~. Requires authentication.
