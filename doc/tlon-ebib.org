...
+ ~tlon-ebib-get-entries~ :: Retrieves all BibTeX entries from the configured API endpoint (~tlon-ebib-api-base-url~). Saves the retrieved entries to the file specified by ~tlon-ebib-file-db~ (using =utf-8-unix= encoding) and opens it. The file will be in =bibtex-mode= due to its =.bib= extension.
