...
+ ~tlon-ebib-check-name~ :: Checks if a given =NAME= exists in the API's database. Requires authentication. Saves the results (existence, conflicts) to the file specified by ~tlon-ebib-file-temp~ (using =utf-8-unix= encoding) and opens it.
...
+ ~tlon-ebib--display-result-buffer~ :: Writes a =TITLE= and the content generated by =FORMATTER-FN= with =DATA= to the file specified by ~tlon-ebib-file-temp~ (using =utf-8-unix= encoding). If optional =RAW-RESPONSE-TEXT= is provided, it is inserted into the buffer before the formatted data. Finally, it opens this file. The =BUFFER-NAME= argument is ignored.
