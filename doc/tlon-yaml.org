...
+ ~tlon-yaml-copy-key~ :: Copies the YAML =key= field value from the current
+  buffer or, when point is on a Markdown file in Dired, from that file, to the
+  kill ring. Signals an error when the file is not Markdown or the field is
+  missing. Returns the copied value.

#+findex: tlon-yaml-get-list
+ ~tlon-yaml-get-list~ :: Returns the YAML list value of KEY from a FILE-OR-BUFFER
+  as a list of strings. If the underlying YAML is a single string, returns a
+  singleton list. Returns nil when the key is absent or empty.
...
+ ~tlon-yaml-insert-field~ :: Interactively prompts for a KEY and VALUE and inserts the corresponding field into the YAML metadata. When KEY is "html_title" and the field is not already present: for articles, the prompt defaults to the BibTeX title of the entry referenced by the article's YAML =key=; for authors and tags, it defaults to the value of the YAML =title= field.
...
+ ~tlon-yaml-insert-translated-tags~ :: Inserts translated tag titles into the `tags` field of a Markdown file, based on the tags of its English counterpart. If a target-language tag file does not exist, it falls back to the glossary by looking up the English tag title and inserting the target-language translation; it reports which tags were translated via the glossary.
...
+ ~tlon-yaml-ko-backfill-original-keys~ :: For a curated list of Korean
  articles, set ~original_key~ by reading ~original_path~, opening the English
  counterpart under =/Users/pablostafforini/Library/CloudStorage/Dropbox/repos/uqbar-en/article/=,
  retrieving its YAML ~key~, and writing it back. Skips files that already have
  ~original_key~ and prints a summary.

#+findex: tlon-yaml-collect-english-tags-in-dir
+ ~tlon-yaml-collect-english-tags-in-dir~ :: Returns the list of unique English
  tag titles found in the English counterparts of all Markdown files in the
  current directory (non-recursive). Intended as a temporary helper.
