  ...
  (let* ((video-file (read-file-name "Select video file: " paths-dir-downloads nil t nil
    ...
    (let* ((request-data (tlon-youtube--prepare-upload-request
      ...
      (with-current-buffer (get-buffer-create "*YouTube Upload Commands*")
        ...
        (insert "--------------------------\n")
        (insert "After running step 1, look for the 'Location:' header in the output.\n")
        (insert "Copy that URL and replace UPLOAD_URL_PLACEHOLDER in this command:\n\n")
        (insert upload-shell-command)
        (insert "\n\n")
        (insert "Files:\n")
        (insert "------\n")
