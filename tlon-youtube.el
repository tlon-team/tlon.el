...
(defun tlon-youtube--upload-video-file (video-file title description privacy)
  ...
  (let* ((access-token (tlon-youtube--get-access-token))
    ...
    ;; Create the multipart body as a unibyte string
    (let* ((crlf "\r\n")
           (metadata-utf8 (encode-coding-string metadata 'utf-8))
           (header1 (concat "--" boundary crlf))
           (header2 (concat "Content-Type: application/json; charset=UTF-8" crlf crlf))
           (header3 (concat crlf "--" boundary crlf))
           (header4 (concat "Content-Type: video/mp4" crlf crlf))
           (footer (concat crlf "--" boundary "--" crlf))
           ;; Read video file as unibyte data
           (video-data (with-temp-buffer
                         (set-buffer-multibyte nil)
                         (insert-file-contents-literally video-file)
                         (buffer-string)))
           ;; Concatenate all parts as unibyte strings
           (request-body (concat (string-as-unibyte header1)
                                (string-as-unibyte header2)
                                (string-as-unibyte metadata-utf8)
                                (string-as-unibyte header3)
                                (string-as-unibyte header4)
                                video-data
                                (string-as-unibyte footer))))
      (message "Debug: request-body is multibyte: %s, size: %d" 
               (multibyte-string-p request-body) (length request-body))
