...
(defun tlon-youtube--upload-video-file (video-file title description privacy)
  ...
  (let* ((access-token (tlon-youtube--get-access-token))
    ...
    (with-temp-buffer
      ...
      (let ((header1 (format "--%s\r\n" boundary))
            (header2 "Content-Type: application/json; charset=UTF-8\r\n\r\n")
            (header3 (format "\r\n--%s\r\n" boundary))
            (header4 "Content-Type: video/mp4\r\n\r\n")
            (footer (format "\r\n--%s--\r\n" boundary)))
        (message "Debug: inserting headers...")
        (insert (string-as-unibyte header1))
        (insert (string-as-unibyte header2))
        (message "Debug: inserting metadata...")
        (insert (string-as-unibyte (encode-coding-string metadata 'utf-8)))
        (insert (string-as-unibyte header3))
        (insert (string-as-unibyte header4))
        (message "Debug: inserting video file...")
        (insert-file-contents-literally video-file)
        (message "Debug: inserting footer...")
        (insert (string-as-unibyte footer))
        (message "Debug: buffer multibyte: %s, buffer size: %d" (multibyte-string-p (buffer-string)) (buffer-size))
