...
Project-local agent skills for Tlön live under this repository's =.claude/skills/=
and =.codex/skills/= directories.

** triage-issues

Scans, prioritizes, and batch-resolves open GitHub issues for
=tlon-team/tlon.el=. It keeps triage, verified local fixes, GitHub publication
and closure, and Org archiving separately scoped. It tracks processed issues
across batches, checks the bot identity and publication results, and archives
only an unambiguous matching Org heading.
