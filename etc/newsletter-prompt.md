...
**Fecha de hoy:** %6$s. Usa esta fecha como referencia para todo juicio sobre qué eventos siguen siendo futuros y qué tiempo verbal corresponde.
...
2. Pégale una mirada a los canales relevantes del Slack de la comunidad de altruismo eficaz e identifica todos los acontecimientos relevantes para el período del que se ocupa este número del boletín, que es el mes de *%1$s*. Los canales relevantes son #general, #oportunidades, #proyectos y #misc. Los acontecimientos relevantes son *eventos* (como conferencias, summits, etc.), *oportunidades* (como ofertas laborales), y *anuncios* (cualquier notificación de interés, como el lanzamiento de un sitio u organización, la publicación de un libro, etc.). Para ver un listado de los canales, utiliza la herramienta "slack_list_channels". Para obtener la fecha, utiliza la herramienta "slack_get_channel_history". **IMPORTANTE: Cuando llames a slack_get_channel_history, debes incluir SIEMPRE el parámetro "oldest" con el valor %3$s (timestamp Unix). Esto limitará los resultados solo a mensajes de los últimos 30 días y evitará exceder el límite de tokens.** El timestamp que te devuelve la herramienta tiene el formato `("ts":"1756389140.470059")` en formato Unix. Tu objetivo es identificar (1) *anuncios* de acontecimientos ocurridos en el mes de *%1$s* y (2) *eventos* y *oportunidades* que ocurrirán **después de la fecha de hoy** (%6$s), normalmente en los próximos 1-3 meses. **Excluye estrictamente cualquier evento u oportunidad cuya fecha de inicio sea anterior a la fecha de hoy: si el evento ya ocurrió, está ocurriendo hoy mismo, o su plazo de inscripción ya cerró, no debe aparecer en el boletín.** Asegúrate de incluir toda la información relevante para cada acontecimiento, como fechas, URLs, descripciones, etc. Además, incluye en cada caso un link al mensaje de Slack de donde obtuviste la información. **Para construir el permalink de un mensaje de Slack, usa el formato `https://altruismo-eficaz.slack.com/archives/{channel_id}/p{ts_sin_punto}`, donde `{channel_id}` es el ID del canal (e.g. `CBJBWNN4X`) y `{ts_sin_punto}` es el campo `ts` del mensaje con el punto decimal eliminado (e.g. si `ts` es `1756389140.470059`, el permalink es `.../p1756389140470059`). NUNCA inventes o aproximes estos valores: usa exclusivamente los datos reales devueltos por la herramienta.**
...
4. Ahora vuelve al borrador y conviértelo en una versión completa y lista para publicación. Para hacerlo, considera tanto los contenidos actuales del borrador como los acontecimientos que identificaste en el paso (2) y las traducciones y correspondientes resúmenes en el paso (3). Si necesitas explorar algún URL, utiliza las herramientas de navegación web ("brave_web_search" y "fetch_markdown"). Asegúrate de que todo el contenido esté en español y que los resúmenes sean concisos y claros. Ten en cuenta que este boletín se publicará el 1 de %2$s, de modo que si haces referencia a un evento posterior a esa fecha, debes usar el tiempo futuro. Al redactar el contenido, no incluyas ningún comentario editorial como "Nota del editor" o "Aquí tienes el contenido".
5. Finalmente, revisa los números previos del boletín que se incluyen al final de este mensaje. **Elimina del borrador cualquier anuncio, evento, oportunidad o traducción que ya haya aparecido en alguno de esos números, aunque su título o redacción sean distintos.** Esta regla aplica con especial rigor a los anuncios sobre lanzamientos de proyectos, organizaciones, sitios web, libros, informes o iniciativas: cada lanzamiento se anuncia una sola vez, en un único número del boletín. Si reconoces el mismo proyecto, organización o publicación (aunque el texto esté reformulado), trátalo como duplicado y descártalo.
