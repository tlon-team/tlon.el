...
(defcustom tlon-batch-abstract-max-file-bytes (* 5 1024 1024)
  "Skip linked files larger than this many bytes during the AI pass.
Converting very large PDFs with `pdftotext' blocks Emacs's main thread
for as long as the conversion runs (potentially minutes).  When a
candidate file exceeds this threshold, the batch driver logs a
\"too large\" message and moves on.  Set to 0 to disable the cap."
  :type 'natnum
  :group 'tlon-bib)
...
;;;###autoload
(defun tlon-fetch-and-set-abstract (&optional interactive-p preserve-existing)
...
When the entry already contains an abstract, prompt the user for
confirmation.  Bypass this prompt if `tlon-abstract-overwrite' is either
`always' or `never'; if so, the new abstract will, or will not, replace
the existing one, respectively.
...
(background, strategy `non-ai') instead of processing the entry at
point.  PRESERVE-EXISTING skips existing abstracts without prompting, including
ones added while fetching."
  (interactive (list t))
  (if (tlon-bib--should-dispatch-to-batch-p interactive-p)
      (tlon-batch-set-abstracts (buffer-file-name) 'non-ai)
    (tlon-fetch-and-set-abstract--single preserve-existing)))

(defun tlon-bib--should-dispatch-to-batch-p (interactive-p)
  "Return non-nil if the calling command should hand off to the batch driver.
INTERACTIVE-P must be non-nil for dispatch to be considered."
  (and interactive-p
       tlon-ai-batch-fun
       (derived-mode-p 'bibtex-mode)
       (buffer-file-name)))

(defun tlon-fetch-and-set-abstract--single (&optional preserve-existing)
  "Fetch and set the abstract at point, honoring PRESERVE-EXISTING."
  (cl-destructuring-bind (get-field set-field) (tlon-bib--field-accessors)
    (let ((found nil))
      (when (tlon-abstract-may-proceed-p preserve-existing)
	(cl-destructuring-bind (doi isbn url)
	    (mapcar (lambda (field) (funcall get-field field))
		    '("doi" "isbn" "url"))
		...
		(progn
		  (with-current-buffer buffer
                    (when (or (and db (not (eq db ebib--cur-db)))
                              (not (equal key (tlon-get-key-at-point))))
                      (user-error "Abstract target entry changed while fetching"))
                    (if (or (not preserve-existing)
                            (tlon-abstract-may-proceed-p t))
                        (progn
		          (funcall set-field "abstract" (tlon-abstract-cleanup value))
                          (message "Set abstract of `%s'" key))
                      (message "Preserved existing abstract of `%s'" key)))
		  (setq found t))
...
(defun tlon-abstract-may-proceed-p (&optional preserve-existing)
  "Return t iff abstract processing may proceed.
PRESERVE-EXISTING refuses to replace a nonempty abstract without prompting."
  (if (derived-mode-p 'bibtex-mode 'ebib-entry-mode)
      (let* ((get-field (tlon-bib--get-field-fn))
	     (abstract (funcall get-field "abstract")))
	(if (and (not (and preserve-existing
	                  (stringp abstract)
	                  (not (string-empty-p (string-trim abstract)))))
	         (or
	     (eq tlon-abstract-overwrite 'always)
	     (not (and (stringp abstract)
	               (not (string-empty-p (string-trim abstract)))))
...
(defconst tlon-batch-abstract--work-buffer-name " *tlon-batch-abstract-work*"
  "Name of the hidden buffer used as the batch driver's scratch copy.
The leading space hides it from the buffer list.")

(defconst tlon-batch-abstract--log-buffer-name "*tlon-batch-abstract-log*"
  "Name of the log buffer where batch progress is written.")
...
(defvar tlon-batch-abstract--non-ai-queue nil
  "Queue of keys awaiting non-AI processing.")

(defvar tlon-batch-abstract--non-ai-total 0
  "Total entries scheduled for the non-AI pass.")

(defvar tlon-batch-abstract--counters nil
  "Plist of running counters for the current batch.
Keys: `:non-ai-set', `:non-ai-fail', `:ai-queued'.")

(defvar tlon-batch-abstract--strategy nil
  "Strategy of the current batch: one of `non-ai', `ai', or `both'.")

(defvar tlon-batch-abstract--generation 0
  "Monotonically increasing counter tagging each AI request.
When the watchdog fires, the counter is incremented so that stale callbacks
...
Stored as a one-element list so that a saved nil is distinguishable from
\"not saved\".")

(defvar tlon-batch-abstract--saved-batch-fun nil
  "Saved value of `tlon-ai-batch-fun' before batch override.
Stored as a one-element list so that a saved nil is distinguishable from
\"not saved\".  While a background batch is active, `tlon-ai-batch-fun'
is set to nil so that stale callbacks from the legacy iteration path no
longer restart themselves.")
...
`*tlon-batch-abstract-log*' (shown with
`tlon-batch-abstract-show-log') and to
`tlon-batch-abstract-progress-file'.  If the AI pass crashes or is
interrupted, it can be resumed with `tlon-batch-abstract-resume'."
  (interactive)
  (when tlon-batch-abstract--active-p
    (user-error "A batch is already running"))
  (let* ((file (or file
		   (and (derived-mode-p 'bibtex-mode) (buffer-file-name))
		   (read-file-name "Bib file: " nil nil t nil
				   (lambda (f)
    ...
    (setq tlon-batch-abstract--counters
	  (list :non-ai-set 0 :non-ai-fail 0 :ai-queued 0)
	  tlon-batch-abstract--queue nil
	  tlon-batch-abstract--total 0
...
(defun tlon-batch-abstract--kick-off-strategy (strategy)
  "Launch the appropriate pipeline for STRATEGY."
  (pcase strategy
    ('non-ai
     (tlon-batch-abstract--non-ai-prepare)
     (run-with-idle-timer 0 nil #'tlon-batch-abstract--non-ai-step
			  #'tlon-batch-abstract--finish-non-ai-only))
    ('ai
     (tlon-batch-abstract--ai-collect-from-work-buffer)
     (setq tlon-batch-abstract--total (length tlon-batch-abstract--queue))
     (tlon-batch-abstract--log "Queued %d entries for AI"
			       tlon-batch-abstract--total)
     (tlon-batch-abstract--progress-save)
     (if (zerop tlon-batch-abstract--total)
...
(defun tlon-batch-abstract-resume ()
  "Resume an AI batch abstract run from the last saved progress file."
  (interactive)
  (when tlon-batch-abstract--active-p
    (user-error "A batch is already running"))
...
;;;###autoload
(defun tlon-batch-abstract-stop ()
  "Cancel the running batch, if any.
The progress file is preserved so the AI pass can later be resumed via
`tlon-batch-abstract-resume'."
  (interactive)
  (if (not tlon-batch-abstract--active-p)
      (message "No batch is running")
    (tlon-batch-abstract--log "Batch stopped by user")
...
;;;###autoload
(defun tlon-batch-abstract-show-log ()
  "Pop to the batch log buffer."
  (interactive)
  (pop-to-buffer (get-buffer-create tlon-batch-abstract--log-buffer-name)))

(defun tlon-batch-abstract--log (fmt &rest args)
  "Append a timestamped line to the batch log buffer.
FMT and ARGS are passed to `format'."
  (let ((line (concat (format-time-string "[%H:%M:%S] ")
		      (apply #'format fmt args)
		      "\n"))
	(buf (get-buffer-create tlon-batch-abstract--log-buffer-name)))
    (with-current-buffer buf
...
(defun tlon-batch-abstract--work-buffer ()
  "Return the hidden work buffer, creating it from the bib file if needed."
  (let ((buf (get-buffer tlon-batch-abstract--work-buffer-name)))
    (unless (and buf (buffer-live-p buf))
      (setq buf (generate-new-buffer tlon-batch-abstract--work-buffer-name))
      (with-current-buffer buf
	(shut-up
	  (insert-file-contents tlon-batch-abstract--bib-file)
	  (delay-mode-hooks (bibtex-mode)))
  ...
  (let ((buf (get-buffer tlon-batch-abstract--work-buffer-name)))
	...
	(let ((coding-system-for-write 'utf-8))
	  (write-region (point-min) (point-max)
			tlon-batch-abstract--bib-file nil 'silent))
	(set-buffer-modified-p nil)))))

(defun tlon-batch-abstract--kill-work-buffer ()
  "Kill the hidden work buffer, if any."
  (let ((buf (get-buffer tlon-batch-abstract--work-buffer-name)))
    (when (buffer-live-p buf) (kill-buffer buf))))

(defun tlon-batch-abstract--sync-user-buffer ()
  "If the user has the bib file open and unmodified, quietly revert it.
If it is modified, leave it alone and log a warning."
  (let ((buf (get-file-buffer tlon-batch-abstract--bib-file)))
    (when (buffer-live-p buf)
      (if (buffer-modified-p buf)
	  (tlon-batch-abstract--log
	   "Warning: %s has unsaved edits; not reverting"
	   (buffer-name buf))
	(with-current-buffer buf
	  (let ((inhibit-message t))
	    (revert-buffer t t t)))))))

(defun tlon-batch-abstract--check-user-buffer ()
  "Signal an error if the user's bib buffer has unsaved changes."
  (let ((buf (get-file-buffer tlon-batch-abstract--bib-file)))
    (when (and buf (buffer-modified-p buf))
      (user-error "Buffer %s has unsaved changes; save or revert it before running batch"
		  (buffer-name buf)))))

(defun tlon-batch-abstract--resume-from-progress (progress)
  "Resume AI processing from PROGRESS plist."
  (let* ((saved-file (plist-get progress :bib-file))
	 (valid-queue (tlon-batch-abstract--filter-completed-keys-in-file
		       (plist-get progress :queue) saved-file)))
    (setq tlon-batch-abstract--bib-file saved-file
	  tlon-batch-abstract--strategy 'ai
	  tlon-batch-abstract--queue valid-queue
	  tlon-batch-abstract--total (+ (plist-get progress :done)
					(length valid-queue))
	  tlon-batch-abstract--ai-done (plist-get progress :done)
	  tlon-batch-abstract--counters
	  (list :non-ai-set 0 :non-ai-fail 0 :ai-queued 0))
    (tlon-batch-abstract--check-user-buffer)
    (tlon-batch-abstract--work-buffer)
    (tlon-batch-abstract--setup)
    (tlon-batch-abstract--log
     "Resuming: %d entries remaining (%d already done, %d now complete)"
     (length valid-queue) tlon-batch-abstract--ai-done
     (- (length (plist-get progress :queue)) (length valid-queue)))
    (if (null valid-queue)
	(tlon-batch-abstract--finish)
      (tlon-batch-abstract--process-ai-queue))
    (message "Batch resumed in background; see %s for progress"
	     tlon-batch-abstract--log-buffer-name)))

(defun tlon-batch-abstract--filter-completed-keys-in-file (keys file)
  "Return the subset of KEYS in FILE that still lack an abstract."
  (with-temp-buffer
...
CONTINUATION is called with no arguments when the queue is empty."
  (cond
   ...
   ((null tlon-batch-abstract--non-ai-queue)
    ...
    (tlon-batch-abstract--sync-user-buffer)
    (tlon-batch-abstract--log
     "Non-AI pass complete: %d set, %d no-match (%d queued for AI)"
     (plist-get tlon-batch-abstract--counters :non-ai-set)
     (plist-get tlon-batch-abstract--counters :non-ai-fail)
     (plist-get tlon-batch-abstract--counters :ai-queued))
   ...
   (t
    (let* ((key (pop tlon-batch-abstract--non-ai-queue))
	   (num (- tlon-batch-abstract--non-ai-total
		   (length tlon-batch-abstract--non-ai-queue))))
      (condition-case err
	  (tlon-batch-abstract--non-ai-process-key key num)
	(error
	 (tlon-batch-abstract--log
...
(defun tlon-batch-abstract--non-ai-process-key (key num)
  ...
  (with-current-buffer (tlon-batch-abstract--work-buffer)
	...
	(tlon-batch-abstract--non-ai-apply-at-point key num)))))

(defun tlon-batch-abstract--non-ai-apply-at-point (key num)
  "With point at KEY's entry, attempt non-AI lookup; log the outcome.
NUM is the 1-based position in the non-AI pass, used for log messages."
  (let* ((doi (ignore-errors (bibtex-extras-get-field "doi")))
	...
	(bibtex-set-field "abstract" (tlon-abstract-cleanup value)))
      ...
      (tlon-batch-abstract--log
       "[non-AI %d/%d] Set %s" num tlon-batch-abstract--non-ai-total key))
     ((and (eq tlon-batch-abstract--strategy 'both)
	   (ignore-errors (ebib-extras-get-text-file)))
      (push key tlon-batch-abstract--queue)
      (cl-incf (cl-getf tlon-batch-abstract--counters :ai-queued))
      (tlon-batch-abstract--log
       "[non-AI %d/%d] Queue for AI: %s"
       num tlon-batch-abstract--non-ai-total key))
     (t
      (cl-incf (cl-getf tlon-batch-abstract--counters :non-ai-fail))
      (tlon-batch-abstract--log
       "[non-AI %d/%d] No match: %s"
       num tlon-batch-abstract--non-ai-total key)))))

(defun tlon-batch-abstract--finish-non-ai-only ()
  "Finish after a non-AI-only strategy completes."
  (tlon-batch-abstract--log "Batch complete (non-AI only)")
  (tlon-batch-abstract--cleanup)
  (tlon-batch-abstract--kill-work-buffer)
...
(defun tlon-batch-abstract--start-ai-pass ()
  "Start the AI pass after the non-AI pass finishes."
  (setq tlon-batch-abstract--queue (nreverse tlon-batch-abstract--queue)
	tlon-batch-abstract--total (length tlon-batch-abstract--queue))
  (tlon-batch-abstract--progress-save)
  (if (zerop tlon-batch-abstract--total)
      (tlon-batch-abstract--finish)
    (tlon-batch-abstract--log "Starting AI pass: %d entries"
			      tlon-batch-abstract--total)
    (tlon-batch-abstract--process-ai-queue)))
...
(defun tlon-batch-abstract--finish ()
  ...
  (tlon-batch-abstract--progress-delete)
  (tlon-batch-abstract--log
   "Batch complete: %d AI abstracts set" tlon-batch-abstract--ai-done)
  (tlon-batch-abstract--cleanup)
  (tlon-batch-abstract--kill-work-buffer)
  (message "Batch complete; see %s"
	   tlon-batch-abstract--log-buffer-name))
...
(defun tlon-batch-abstract--request-for-key (key num gen)
  ...
  (with-current-buffer (tlon-batch-abstract--work-buffer)
    (save-excursion
	  ...
	  (progn
	    (tlon-batch-abstract--log
	     "[AI %d/%d] Key not found: %s"
	     num tlon-batch-abstract--total key)
	    (tlon-batch-abstract--schedule-next))
	(tlon-batch-abstract--dispatch-at-point key num gen)))))

(defun tlon-batch-abstract--dispatch-at-point (key num gen)
  "With point on KEY's entry, dispatch the AI request.
NUM is the entry number and GEN the generation counter."
...
NUM is the entry number, GEN the generation counter, FILE the text file,
PROMPT the AI prompt, and LANG-CODE the language code."
  (cond
   ((not (and file prompt))
    (tlon-batch-abstract--log
     "[AI %d/%d] Skip %s (no prompt for %s)"
     num tlon-batch-abstract--total key (or lang-code "?"))
    (tlon-batch-abstract--schedule-next))
   ((tlon-batch-abstract--file-too-large-p file)
    (tlon-batch-abstract--log
     "[AI %d/%d] Skip %s (file too large: %s)"
     num tlon-batch-abstract--total key
     (file-size-human-readable (file-attribute-size (file-attributes file))))
    (tlon-batch-abstract--schedule-next))
   (t
    (tlon-batch-abstract--send-request key num gen file prompt))))

(defun tlon-batch-abstract--file-too-large-p (file)
  "Return non-nil if FILE is over `tlon-batch-abstract-max-file-bytes'."
  (and (> tlon-batch-abstract-max-file-bytes 0)
       (file-exists-p file)
       (> (file-attribute-size (file-attributes file))
	  tlon-batch-abstract-max-file-bytes)))

(defun tlon-batch-abstract--send-request (key num gen file prompt)
  "Read FILE, fire the gptel request for KEY (entry NUM, generation GEN).
PROMPT is the summarization prompt."
  (let ((string (ignore-errors (tlon-get-file-as-string file))))
    (if (not string)
	(progn
	  (tlon-batch-abstract--log
	   "[AI %d/%d] Could not read file for %s"
	   num tlon-batch-abstract--total key)
	  (tlon-batch-abstract--schedule-next))
      (tlon-batch-abstract--log
       "[AI %d/%d] %s (gen %d)..." num tlon-batch-abstract--total key gen)
...
GEN is the generation counter at the time the request was issued."
  (lambda (response info)
	...
	(tlon-batch-abstract--log
	 "[AI %d/%d] Stale callback for %s (gen %d, now %d)"
...
(defun tlon-batch-abstract--handle-success (key num response)
  ...
  (condition-case err
	...
	(tlon-batch-abstract--log
	 "[AI %d/%d] Set %s" num tlon-batch-abstract--total key))
    (error
     (tlon-batch-abstract--log
      "[AI %d/%d] Callback error for %s: %S"
      num tlon-batch-abstract--total key err))))

(defun tlon-batch-abstract--set-abstract-in-work-buffer (key response)
  "Write RESPONSE to the `abstract' field of KEY in the work buffer."
  (with-current-buffer (tlon-batch-abstract--work-buffer)
    (save-excursion
      (goto-char (point-min))
      (unless (bibtex-search-entry key)
	(error "Could not find entry for key %s in work buffer" key))
      (shut-up (bibtex-set-field "abstract" response)))))

(defun tlon-batch-abstract--handle-retry (key num http-status)
  "Schedule KEY (entry NUM) for retry after HTTP-STATUS error."
  (tlon-batch-abstract--log
   "[AI %d/%d] Retryable (%s) for %s, retry after %ds"
...
HTTP-STATUS and INFO provide error details."
  (tlon-batch-abstract--log
   "[AI %d/%d] Failed for %s (status: %s, error: %S)"
   num tlon-batch-abstract--total key
...
Neutralizes `tlon-ai-batch-fun' so that callbacks of the legacy
per-entry iteration path (`tlon-ai-batch-continue') cannot restart
themselves while the background driver runs."
  ...
  (setq tlon-batch-abstract--saved-batch-fun (list tlon-ai-batch-fun))
  (setq tlon-ai-batch-fun nil)
...
(defun tlon-batch-abstract--cleanup ()
  ...
  (when tlon-batch-abstract--saved-batch-fun
    (setq tlon-ai-batch-fun (car tlon-batch-abstract--saved-batch-fun))
    (setq tlon-batch-abstract--saved-batch-fun nil))
  ...
  (let ((delay tlon-batch-abstract--backoff-delay))
    (when (>= tlon-batch-abstract--consecutive-errors
      ...
      (setq delay tlon-batch-abstract-circuit-breaker-pause
	    ...
	    tlon-batch-abstract--backoff-delay 1)
      (tlon-batch-abstract--log
       "Circuit breaker tripped, pausing %ds" delay))
...
(defun tlon-batch-abstract--watchdog-fired ()
  ...
  (cl-incf tlon-batch-abstract--consecutive-errors)
  (tlon-batch-abstract--log
   "Watchdog fired (gen %d), advancing" tlon-batch-abstract--generation)
  (tlon-batch-abstract--schedule-next))
