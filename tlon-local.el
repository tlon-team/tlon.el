      ...
      (let* ((labels (alist-get 'stream stream))
        ...
        (dolist (v values)
          (let ((ts (tlon-local--ns-to-timestr (nth 0 v)))
                (line (nth 1 v)))
            (tlon-local--insert-log-row ts line src-label lang)))))
    (insert "\n")))
...
(defun tlon-local--insert-log-row (ts line src-label lang)
  "Insert one row with TS, SOURCE-LABEL and LINE for LANG.
