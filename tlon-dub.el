...
(defconst tlon-dub-propagate-timestamps-prompt-template
  "You will be given two inputs:\n1. A JSON transcript with detailed
timestamps (potentially word-level) generated by WhisperX.\n2. A human-edited
plain text transcript of the same audio. This transcript is more accurate in
terms of wording, but has no timestamps.\n\nYour task is to propagate the
timestamps from the WhisperX JSON to the human-edited transcript.\nOutput the
human-edited transcript, but with a timestamp at the beginning of each new
paragraph. Insert the timestamp on a separate line. Align the timestamps as
accurately as possible to the human-edited text.\n\nWhisperX
JSON:\n```json\n%s\n```\n\nHuman-edited
transcript:\n```text\n%s\n```\n\nTimestamped human-edited transcript:Prompt
template for AI timestamp propagation."
