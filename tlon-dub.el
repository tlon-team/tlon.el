...
(defconst tlon-dub-propagate-timestamps-prompt-template
  (concat
   "You will be given two inputs:\n"
   "1. A JSON transcript with detailed timestamps (potentially word-level) generated by WhisperX.\n"
   "2. A human-edited plain text transcript of the same audio. This transcript is more accurate in terms of wording.\n\n"
   "Your task is to propagate the timestamps from the WhisperX JSON to the human-edited transcript.\n"
   "Output the human-edited transcript, but with VTT-style timestamps (HH:MM:SS.mmm --> HH:MM:SS.mmm) inserted on separate lines *before* the corresponding text segments. "
   "Align the timestamps as accurately as possible to the human-edited text. Ensure every part of the human-edited transcript is covered by a timestamped segment.\n\n"
   "WhisperX JSON:\n"
   "```json\n%s\n```\n\n"
   "Human-edited transcript:\n"
   "```text\n%s\n```\n\n"
   "Timestamped human-edited transcript (VTT-style, ensure complete coverage of human text):")
  "Prompt template for AI timestamp propagation.")
