...
(defun tlon--get-wayback-machine-url (url callback &optional variant)
  "Fetch an archived version of URL from the Wayback Machine asynchronously.
Call CALLBACK with (ARCHIVE-URL ORIGINAL-URL).  ARCHIVE-URL is nil if no
...
VARIANT selects which API endpoint to query:
  ...
  (let* ((variant (or variant 'latest))
         (api-url
          (pcase variant
            ('latest
             (format "https://archive.org/wayback/available?url=%s"
                     (url-hexify-string url)))
            ('earliest
             (format "https://web.archive.org/cdx/search/cdx?url=%s&statuscode=200&limit=1"
                     (url-hexify-string url)))))
         (label (if (eq variant 'latest) "latest" "earliest")))
    (message "Fetching %s working archive for %s..." label url)
    (url-retrieve
     api-url
         ...
         (unwind-protect
             (with-current-buffer buffer
               (if (plist-get status :error)
                   (message "Wayback Machine request for %s failed: %S"
                              ...
                              (when (and timestamp original-url-from-api)
                                (setq archive-url
                                      (tlon--normalize-wayback-url
                                       (format "https://web.archive.org/web/%s/%s"
                                               timestamp original-url-from-api)))))
...
(defvar tlon-lychee-whitelist-file
  (file-name-concat (if (boundp 'elpaca-sources-directory)
                        elpaca-sources-directory
                      elpaca-repos-directory)
                    "tlon/etc/tlon-url-lychee-whitelist.txt")
...
(defun tlon-get-archived-url (url &optional variant)
  "Fetch an archived version of URL from the Wayback Machine and copy it.
VARIANT is `latest' (the default) or `earliest'.  The archive URL is copied to
  ...
  (let* ((variant (or variant 'latest))
    ...
    (tlon--get-wayback-machine-url
     ...
     (lambda (archive-url original-url)
       (if archive-url
           (progn
             (message "%s working archive: %s" label archive-url)
...
Also, copy the URL to the kill ring."
  (interactive "sURL: ")
  (tlon-get-archived-url url 'latest))
...
Also, copy the URL to the kill ring."
  (interactive "sURL: ")
  (tlon-get-archived-url url 'earliest))
...
ARCHIVE-URL is nil if no archive is found or an error occurs."
  (tlon--get-wayback-machine-url url callback 'earliest))
...
(defun tlon-lychee--format-action-summary (action-counts-ref processed-links-count-ref
                                                            total-dead-links prefix)
  "Format and display dead-link processing summary.
ACTION-COUNTS-REF is the plist of action counts.  PROCESSED-LINKS-COUNT-REF
is the count ref.  TOTAL-DEAD-LINKS is the total found.  PREFIX is a string
like \"Lychee dead link processing complete.\" or \"Aborted.\"."
  (message "%s Processed %d out of %d dead links found (including whitelisted).\nActions taken: %d archived, %d replaced, %d removed, %d whitelisted, %d skipped."
           prefix
           (car processed-links-count-ref) total-dead-links
           (cl-getf action-counts-ref :archived)
           (cl-getf action-counts-ref :replaced)
           (cl-getf action-counts-ref :removed)
           (cl-getf action-counts-ref :whitelisted)
           (cl-getf action-counts-ref :skipped)))

(defun tlon-lychee--handle-replace-action (url full-file-path filename
                                                dead-links-queue remaining-links total-dead-links
                                                action-counts-ref processed-links-count-ref
                                                stderr-content)
  "Handle the replace action for dead link URL in FULL-FILE-PATH (FILENAME).
Prompts the user for a replacement URL, validates it, performs the replacement,
and continues to the next link.  DEAD-LINKS-QUEUE is the full queue (for
re-prompting on invalid input).  REMAINING-LINKS, TOTAL-DEAD-LINKS,
ACTION-COUNTS-REF, PROCESSED-LINKS-COUNT-REF, and STDERR-CONTENT are passed
through to `tlon-lychee--process-next-dead-link'."
...
Wait for user input before proceeding to the next link."
  (if (null dead-links-queue)
      (tlon-lychee--format-action-summary action-counts-ref processed-links-count-ref
                                          total-dead-links "Lychee dead link processing complete.")
            ...
            (tlon--get-wayback-machine-url
             ...
             (lambda (archive-url original-dead-url)
               (tlon-lychee--handle-archive-response
                ...
                stderr-content)))))
         ((eq action ?r)
          (tlon-lychee--handle-replace-action url full-file-path filename
                                              dead-links-queue remaining-links total-dead-links
                                              action-counts-ref processed-links-count-ref
                                              stderr-content))
         ((eq action ?w)
