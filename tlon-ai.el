...
(defcustom tlon-ai-replace-citations-model
  '("Gemini" . gemini-2.5-pro-preview-06-05)
  "Model to use for replacing citations (`tlon-ai-replace-citations-in-file').
The value is a cons cell whose car is the backend and whose cdr is the model
itself. See `gptel-extras-ai-models' for the available options. If nil, use the
default `gptel-model'."
  :type '(cons (string :tag "Backend") (symbol :tag "Model"))
...
4. If you succeed in finding an identifier, use the `add_bib_entry` tool to add a new entry to the bibliography file. The `bibfile` parameter for this tool MUST be '%s'. The `identifier` parameter should be the URL, DOI, or ISBN you found.
5. After adding the entry, replace the original citation string in the text with the format `<Cite bibKey=\"KEY\" />`, where `KEY` is the BibTeX key generated by the `add_bib_entry` tool. You can find this key in the bibfile specified in the variable above, under (4). For example, replace '{!Smith, 2019$}' with '<Cite bibKey=\"Smith2019EarlyCovidVaccines\" />'.
...
keys, and replace them with `<Cite bibKey=\"KEY\" />` tags."
  ...
  (let* ((file (read-file-name "File to process: "))
    ...
    (tlon-make-gptel-request prompt nil #'tlon-ai-replace-citations-callback tlon-ai-replace-citations-model t nil tools)))
...
(transient-define-infix tlon-ai-infix-select-replace-citations-model ()
